                ...
                <div class="card text-white" id="carReservacion">
                    ...
                    <div class="card-body text-dark">
                        <form method="post" @submit.prevent="guardarReservacion" @reset="nuevoReservacion">
                            <div class="row p-1">
                                <div class="col col-md-3">Cliente:</div>
                                <div class="col">
                                    <v-select-Cliente title="Seleccione el Cliente" v-model="Reservacion.Cliente" :options="Clientes" required class="form-control"/>
                                </div>
                            </div>
                            <div class="row p-1">
    ...
    export default {
        ...
        methods:{
            ...
            obtenerDatos(valor=''){
                ...
                //obtener Clientes 
                let storeCliente = this.abrirStore('Cliente', 'readonly'),
                    dataCliente = storeCliente.getAll();
                dataCliente.onsuccess = e=>{
                    if( dataCliente.result.length<=0 ){
                        fetch(`Cliente`, 
                            {credentials: 'same-origin'})
                            .then(res=>res.json())
                            .then(dataCliente=>{
                                ...
                                dataCliente.map(Cliente=>{
                                    ...
                                    query.onsuccess = e=>{
                                        console.log(`Reservacion ${Cliente.nombre} guardado`);
                                    };
                                    query.onerror = e=>{
                    ...
                    this.Clientes = dataCliente.result.map(Cliente=>{
                        return {
                            id : Cliente.id,
